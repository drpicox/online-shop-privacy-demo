...
import { selectClientActions, selectLastAction, selectClientLastAction, selectHasClientState } from '@/store/viewer';
...
export default function ActionViewer({ clientId, showLastActionOnly = false }: ActionViewerProps) {
  ...
  const hasClientState = useViewerSelector((state) => 
    clientId ? selectHasClientState(state, clientId) : false
  );
