...
function ViewportVisualizer({
  clientId,
  compact = false,
}: ViewportVisualizerProps) {
  const clientState = useViewerSelector((state) =>
    selectClientState(state, clientId)
  );
  ...
  useEffect(() => {
    ...
    // Get current ref value to use in cleanup
  ...
  return (
    <div
      ref={containerRef}
      className={`bg-gray-100 ${compact ? "p-2" : "p-4"} rounded-lg`}
    >
      {!compact && (
        <div className="mb-4">
          ...
          <div className="flex items-center mb-3 bg-white rounded border border-gray-300 p-1 pr-2">
            <div className="flex-shrink-0 flex items-center mr-1">
              <svg
                ...
                className="h-4 w-4 text-gray-600 mx-1"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
      ...
      {false && (
        <div className="flex justify-center mt-1">
          <div className="flex" style={{ width: `${scaledWidth}px` }}>
            ...
            <div className="relative mr-3 h-2 bg-gray-300 rounded-full flex-grow">
              <div
      ...
      {!compact && (
        <div className="mt-4 text-xs text-gray-500">
          <p>
            Last updated:{" "}
            {new Date(clientState?.timestamp ?? "").toLocaleTimeString()}
          </p>
        </div>
      )}
      {compact && (
